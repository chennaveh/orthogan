...
from model.orthogan import OrthoGAN
...
def main():
    params = load_params()
    orthogan = OrthoGAN(params)
    orthogan.load_checkpoints()
    orthogan.subspace_model.to(params.device)
    orthogan.subspace_model.eval()
    ...
    for fname in glob.glob('./latent_vectors/*'):
        ...
        for im_idx, weights in enumerate(GEN_PARAMS[im_name]['weights']):
            directions = orthogan.get_subspace_directions(GEN_PARAMS[im_name]['attr'])
            ...
            img = orthogan.stylegan(edit_tensor, latent_space_type='WP')['image']
